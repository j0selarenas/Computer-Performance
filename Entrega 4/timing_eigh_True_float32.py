...
def eigenValues(caso, matrizNxN):
    ...
    if caso == 1:
        ...
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = True, turbo = "ev")
    elif caso == 3:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = True, turbo = 'evd')
    elif caso == 4:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = True, turbo = 'evr')
    else:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = True, turbo = 'evx')
    t2 = perf_counter()
    dt = t2 - t1
    return(dt)
...
def exportar(caso, datos_N, datos_dt, dtype):
    nombre_archivo = "rendimiento_eigh_caso_" + str(caso) + "_True_float32.txt"
    if dtype == np.double:
