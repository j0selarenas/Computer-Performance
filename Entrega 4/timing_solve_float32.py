...
def eigenValues(caso, matrizNxN):
    ...
    if caso == 1:
        ...
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = False, overwrite_b = True, turbo = "ev")
    elif caso == 3:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = False, overwrite_b = True, turbo = 'evd')
    elif caso == 4:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = False, overwrite_b = True, turbo = 'evr')
    else:
        w, h = sp.linalg.eigh(matrizNxN, overwrite_a = False, overwrite_b = True, turbo = 'evx')
    t2 = perf_counter()
    dt = t2 - t1
    return(dt)
...
def exportar(caso, datos_N, datos_dt, dtype):
    nombre_archivo = "rendimiento_solve_caso_" + str(caso) + "_float32.txt"
    if dtype == np.double:
...
def repeticionN(caso, valores_N, dtype):
    ...
    for i in range(len(valores_N)):
        ...
        lista_dt.append(solveSciPy(caso, valores_N[i], matrizNxN))
...
def main():
    ...
    caso_6 = promedio(corridas(casos[5], valores_N, dtype))
    exportar(casos[5], valores_N, caso_6, dtype)
    
    caso_7 = promedio(corridas(casos[6], valores_N, dtype))
    exportar(casos[6], valores_N, caso_7, dtype)
