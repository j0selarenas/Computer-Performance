...
def inverseNumPy(matriz):
    t1 = perf_counter()
    matrizINV = np.linalg.inv(matriz)
    t2 = perf_counter()
    dt_inversion = t2 - t1
    uso_memoria = matrizINV.nbytes
    datos_NumPy = [matrizINV, dt_inversion, uso_memoria]
    return(datos_NumPy)

def exportar(datos_N, datos_dt, datos_memoria):
    with open('rendimiento_caso_1_single.txt', 'w') as f:
...
def main():
    ...
    while repeticiones <= 10:
        ...
        for i in range(len(valores_N)):
            ...
            memoria_inv = inverseNumPy(matrizNxN)[2]
            memoria_total = matrizNxN.nbytes + memoria_inv
            dt_inv = inverseNumPy(matrizNxN)[1]
