...
def inverseSciPy(matrizNxN, caso):
    t1 = perf_counter()
    if caso == 'llena':
        x = sp.linalg.inv(matrizNxN, overwrite_a=True)
    elif caso == 'dispersa':
        x = lin.inv(matrizNxN)
    t2 = perf_counter()
    dt = t2 - t1
...
def exportar(datos_N, dt_ensamblaje, dt_solucion, caso):
    with open("rendimiento_inv_"+caso+".txt", 'w') as f:
...
def repeticionN(valores_N, dtype, caso):
    ...
    for i in range(len(valores_N)):
        ...
        if caso == 'llena':
            ...
            matrizNxN = sparse.csc_matrix(matriz)
        t2 = perf_counter()
        matrizInv = inverseSciPy(matrizNxN, caso)
        t3 = perf_counter()
        dt_ensamblaje = t2 - t1
        dt_solucion = t3 - t2
