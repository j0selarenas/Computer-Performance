...
def exportar(datos_N, dt_ensamblaje, dt_matmul):
    with open("rendimiento_matmul_llena.txt", 'w') as f:
...
def repeticionN(valores_N, dtype):
    ...
    for i in range(len(valores_N)):
        t1 = perf_counter()
        matrizLlenaA = laplacianaLlena(valores_N[i], dtype)
        matrizLlenaB = laplacianaLlena(valores_N[i], dtype)
        t2 = perf_counter()
        matrizLlenaC = matmul(matrizLlenaA, matrizLlenaB, 'llena')
        t3 = perf_counter()
        dt_ensamblaje = t2 - t1
        dt_matmul = t3 - t2
